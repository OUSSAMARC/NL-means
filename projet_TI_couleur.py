...
# Répertoire actuel du script
script_directory = os.path.dirname(os.path.abspath(__file__))
# Répertoire parent
parent_directory = os.path.abspath(os.path.join(script_directory, os.pardir))
# Répertoire des images
images_directory = os.path.join(parent_directory, 'Images et vidéos', 'oeil.png') 

# Ouverture de l'image avec PIL
img = Image.open(images_directory)

# Conversion de l'image en tableau NumPy et normalisation des valeurs dans la plage [0, 1]
img_color = np.array(img) / 255.0

# Ajout de bruit gaussien additif à l'image 
row, col, ch = img_color.shape
noisex = np.random.normal(0, 0.05, (row, col, ch)).astype(np.float64)
noisy = img_color + noisex

# Extraction des canaux de couleur individuels
img_channels = [noisy[:, :, i] for i in range(3)]
...
# Application de l'algorithme à chaque canal et ajout à filtered_channels
filtered_channels = []
for channel in img_channels:
    filtered_channels.append(apply_algorithm_to_channel(channel))
# L'image finale débruitée
